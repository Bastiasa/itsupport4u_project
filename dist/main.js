...
async function scrapCredly(browser) {
    ...
    const badges = await page.$$eval(CREDLY_CERTIFICATION_CONTAINER_SELECTOR, (elements, { imageClass, nameClass }) => {
        return elements.map(element => {
            const IMAGE_ELEMENT = element.querySelector(imageClass);
            const NAME_ELEMENT = element.querySelector(nameClass);
            return {
    ...
    await promises_1.default.writeFile("webscrapping/credly_badges.json", JSON.stringify(badges), "utf8");
}
async function scrapWallet(browser) {
    ...
    let CERTIFICATIONS = await page.$$eval(WALLET_CERTIFICATION_CONTAINER_SELECTOR, (elements, { imageSelector, nameSelector, linkSelector, customCoverSelector, screenshotCoverSelector }) => {
        return elements.map((certificationContainer, index) => {
            const NAME_ELEMENT = certificationContainer.querySelector(nameSelector);
            const LINK_ELEMENT = certificationContainer.querySelector(linkSelector);
    ...
    await promises_1.default.writeFile("webscrapping/wallet_badges.json", JSON.stringify(CERTIFICATIONS), 'utf8');
}
async function main() {
    await promises_1.default.mkdir("webscrapping/", { recursive: true });
